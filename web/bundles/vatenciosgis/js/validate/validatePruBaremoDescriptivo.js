$(document).ready(function(){
    $("[name='pru_baremo_descriptivo']").validate({
        messages: {
            rht_rhtonlinebundle_sispaistype_nombre: {
    ...
    $("form[name='pru_baremo_descriptivo'] select").change(function(){
        $("form[name='pru_baremo_descriptivo'] div").find("label.error").each(function(index,element){
    ...
    $("form[name='pru_baremo_descriptivo'] input,select,textarea").blur(function(){
        ...
        //console.log(boton_crear,$(".ui-dialog-buttonset button.emergente"),$(".ui-dialog-buttonset"));
        ...
        if($("form[name='pru_baremo_descriptivo'] .error").text() && !visible){
        ...
        if((boton_crear.hasClass('emergente') && $("form[name='pru_baremo_descriptivo']").parent().attr("id") === "show_form_emergente")
            || (!boton_crear.hasClass('emergente') && $("form[name='pru_baremo_descriptivo']").parent().attr("id") === "show_form")){
            hacer_focus = true;
        }else{
            hacer_focus = false;
        }
        ...
        boton_crear.focus(function(){
            if(hacer_focus){
                ...
                $("[name='pru_baremo_descriptivo']").submit();
                if($("form[name='pru_baremo_desciptivo'] .error").text() && !visible){
                    boton_crear.attr('disabled',true);
                }else{
    ...
    if($("form[name='pru_baremo_descriptivo']").parent().attr("id") !== "show_form_emergente"){
        boton_nuevo();
    }
});